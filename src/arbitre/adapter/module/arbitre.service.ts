...
export class ArbitreService implements IArbitreService {
  ...
  async add(
    data: ArbitreAccountDto,
    file?: Express.Multer.File,
  ): Promise<Arbitre> {
    try {
      const { name } = data;
      const existed = await this.arbitresRepository.arbitres.findOneBy({
        name,
      });
      if (existed) throw new ConflictException('arbitre already exists');
      ...
      const arbitre = await ArbitreFactory.create({
        ...data,
        avatar: avatarUrl,
      });
  ...
  async remove(id: string): Promise<boolean> {
    try {
      ...
      if (arbitres) {
        return await this.arbitresRepository.arbitres
          .remove(arbitres)
          .then(() => true);
      }
      return false;
