...
export class TransactionController implements ITransactionController {
  ...
  async all(@Query() options: PaginationOptionsDto): Promise<PaginationResultDto<Transaction>> {
    const result = await this.transactionService.fetchAll(options);
    
    const mappedItems = result.items?.map((transaction) => TransactionFactory.getTransaction(transaction));
