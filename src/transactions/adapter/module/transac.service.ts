...
export class TransactionService implements ITransactionService {
    ...
    try {
      ...
      if (amount < 500)
        throw new BadRequestException(
          'Le montant doit être super ou égal à 500frs',
        );

      const verifyPass = await HashFactory.isRightPwd(pass, adminE.password);
      if (!verifyPass)
        throw new BadRequestException('Mot de passe admin incorrect');
  ...
  async edit(data: UpdateTransactionDTO): Promise<Transaction> {
    try {
      const { id } = data;
      const transaction =
        id && (await this.transactionRepository.transactions.findOneByID(id));
      if (transaction) {
      ...
      this.logger.error(
        error.message,
        'ERROR::TransactionService.editTransaction',
      );
  ...
  async remove(id: string): Promise<boolean> {
    try {
      const transaction =
        await this.transactionRepository.transactions.findOneByID(id);
      if (transaction) {
        return await this.transactionRepository.transactions
          .remove(transaction)
          .then(() => true);
      }
      return false;
