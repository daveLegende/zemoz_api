...
export class TicketService implements ITicketService {
  ...
  private async generateTicketIdentifier(
    ...
    const barcode =
      `TKT${timestamp}${Math.random().toString().slice(2, 9)}`.substring(0, 13);
  ...
  async edit(data: UpdateTicketDTO): Promise<Ticket> {
    try {
      ...
      const ticket =
        id &&
        (await this.ticketRepository.tickets.findOne({
          where: { id: id },
          relations: { user: true },
        }));
      ...
      if (ticket) {
        ...
        if (ticket.duree === TicketDuration.PHASE_POULE) {
          ...
          if (!isGroupMatchDay) {
            throw new BadRequestException(
              "Aucun match de poule prévu aujourd'hui",
            );
          }
          ...
          if (
            ticket.lastScanDate &&
            this.isSameDay(ticket.lastScanDate, currentDate)
          ) {
            throw new BadRequestException(
              "Le ticket a déjà été scanné aujourd'hui",
            );
          }
  ...
  async areGroupStagesOver(): Promise<boolean> {
    ...
    const allMatchesFinished = matchs.every(
      (match) => match.etat === MatchState.TERMINER,
    );
  ...
  async remove(id: string): Promise<boolean> {
    try {
      ...
      if (ticket) {
        return await this.ticketRepository.tickets
          .remove(ticket)
          .then(() => true);
      }
      return false;
  ...
  async scanTicket(qrCode: string): Promise<Ticket> {
    ...
    switch (ticket.duree) {
      ...
      case TicketDuration.PHASE_POULE:
        ...
        if (ticket.lastScanDate && this.isSameDay(ticket.lastScanDate, now)) {
          throw new BadRequestException(
            "Ce ticket a déjà été scanné aujourd'hui",
          );
        }
        ...
        if (ticket.position === TicketPosition.SORTIE) {
          ticket.position = TicketPosition.ENTREE;
          this.logger.log(
            `Ticket ${ticket.id}: Entrée phase poule à ${now.toISOString()}`,
          );
        } else {
          ticket.position = TicketPosition.SORTIE;
          this.logger.log(
            `Ticket ${ticket.id}: Sortie phase poule à ${now.toISOString()}`,
          );
        }
        ticket.lastScanDate = now;
      ...
      case TicketDuration.TOURNOI_COMPLET:
        ...
        if (ticket.position === TicketPosition.SORTIE) {
          ticket.position = TicketPosition.ENTREE;
          this.logger.log(
            `Ticket ${ticket.id}: Entrée tournoi complet à ${now.toISOString()}`,
          );
        } else {
          ticket.position = TicketPosition.SORTIE;
          this.logger.log(
            `Ticket ${ticket.id}: Sortie tournoi complet à ${now.toISOString()}`,
          );
        }
        ticket.lastScanDate = now;
