...
export class TicketService implements ITicketService {
  ...
  async fetchAll(options: PaginationOptionsDto): Promise<PaginationResultDto<Ticket>> {
    try {
      const [tickets, total] = await this.ticketRepository.tickets.findAndCount({
        skip: options.skip,
        take: options.limit,
        relations: { user: true },
        order: { createdAt: 'DESC' }
      });
      return new PaginationResultDto(tickets, total, options.page, options.limit);
    } catch (error) {
