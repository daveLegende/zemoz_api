...
export class TicketController implements ITicketController {
  ...
  async all(@Query() options: PaginationOptionsDto): Promise<PaginationResultDto<Ticket>> {
    const result = await this.ticketService.fetchAll(options);
    const mappedItems = result.items.map((Ticket) => TicketFactory.getTicket(Ticket));
