...
export class CouponBetService implements ICouponBetService {
  ...
  async fetchAll(options: PaginationOptionsDto): Promise<PaginationResultDto<CouponBet>> {
    const [couponBets, total] = await this.couponBetsRepository.couponBets.findAndCount({
      skip: options.skip,
      take: options.limit,
