...
export class BetService implements IBetService {
  ...
  async fetchAll(options: PaginationOptionsDto): Promise<PaginationResultDto<Bet>> {
    try {
      const [bets, total] = await this.betsRepository.bets.findAndCount({
        skip: options.skip,
        take: options.limit,
        relations: { match: true, competition: true },
        order: { createdAt: 'DESC' }
      });
      return new PaginationResultDto(bets, total, options.page, options.limit);
    } catch (error) {
