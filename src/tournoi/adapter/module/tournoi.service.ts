...
export class TournoiService implements ITournoiService {
  ...
  async fetchAll(options: PaginationOptionsDto): Promise<PaginationResultDto<Tournoi>> {
    try {
      const [tournois, total] = await this.tournoiRepository.tournois.findAndCount({
        skip: options.skip,
        take: options.limit,
        order: { name: 'ASC' }
      });
      return new PaginationResultDto(tournois, total, options.page, options.limit);
    } catch (error) {
