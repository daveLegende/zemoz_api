...
import * as multer from 'multer';
  ...
  export class MatchController implements IMatchController {
    ...
    @ApiOperation({
      summary: 'matchs list',
      description: 'Fetch all matchs in the DB with pagination',
    })
    // @ApiResponse({ type: [MatchAccountDTO] })
    async all(@Query() options: PaginationOptionsDto): Promise<PaginationResultDto<Match>> {
      const result = await this.matchService.fetchAll(options);
      
      const mappedItems = result.items.map((match) => MatchFactory.getMatch(match));
