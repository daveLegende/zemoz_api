...
export class MatchService implements IMatchService {
  ...
  async fetchAll(options: PaginationOptionsDto): Promise<PaginationResultDto<Match>> {
    try {
      let relations: any = {
        home: true,
        away: true,
      ...
      if (options.relations !== undefined) {
        if (options.relations === '') {
          ...
          relations = options.relations.split(',').reduce((acc, rel) => {
            acc[rel.trim()] = true;
            return acc;
          }, {});
        }
      ...
      const [matches, total] = await this.matchRepository.matchs.findAndCount({
        skip: options.skip,
        take: options.limit,
        relations: relations,
        order: { date: 'DESC' }
      ...
      return new PaginationResultDto(matches, total, options.page, options.limit);
    } catch (error) {
