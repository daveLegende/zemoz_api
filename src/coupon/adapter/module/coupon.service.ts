...
export class CouponService {
  ...
  // Récupérer tous les coupons avec relations et pagination
  async fetchAll(options: PaginationOptionsDto): Promise<PaginationResultDto<Coupon>> {
    try {
      const [coupons, total] = await this.couponsRepository.coupons.findAndCount({
        skip: options.skip,
        take: options.limit,
        relations: {
          user: true,
      ...
      this.logger.error(error.message, 'ERROR::CouponService.fetchAll');
      throw error;
    }
  }
