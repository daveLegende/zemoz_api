...
export class MVPService implements IMVPService {
  ...
  async fetchAll(options: PaginationOptionsDto): Promise<PaginationResultDto<MVP>> {
    try {
      const [mvps, total] = await this.mvpRepository.mvps.findAndCount({
        skip: options.skip,
        take: options.limit,
        relations: {
          user: true,
      ...
      return new PaginationResultDto(mvps, total, options.page, options.limit);
    } catch (error) {
