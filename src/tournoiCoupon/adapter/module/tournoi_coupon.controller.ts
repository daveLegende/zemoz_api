...
export class TournoiCouponController implements ITournoiCouponController {
  ...
  async all(@Query() options: PaginationOptionsDto): Promise<PaginationResultDto<TournoiCoupon>> {
    const result = await this.tournoiCouponService.fetchAll(options);
    
    const mappedItems = result.items?.map((coupon) => TournoiCouponFactory.getCoupon(coupon));
