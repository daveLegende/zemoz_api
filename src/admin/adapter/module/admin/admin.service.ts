...
export class AdminService implements IAdminService {
  ...
  async fetchAll(options: PaginationOptionsDto): Promise<PaginationResultDto<Admin>> {
    try {
      const [admins, total] = await this.adminRepository.admins.findAndCount({
        skip: options.skip,
        take: options.limit,
        order: { createdAt: 'DESC' }
      });
      return new PaginationResultDto(admins, total, options.page, options.limit);
    } catch (error) {
