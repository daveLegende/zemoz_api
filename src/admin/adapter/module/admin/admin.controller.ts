...
export class AdminController implements IAdminController {
  ...
  async all(@Query() options: PaginationOptionsDto): Promise<PaginationResultDto<Admin>> {
    const result = await this.adminService.fetchAll(options);
    
    const mappedItems = result.items?.map((admin) => AdminFactory.getAdmin(admin));
